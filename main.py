...
with tab5:
    ...
    # Defaults for fixed + single variable filter
    defaults = {
        # FIXED filters
        "tab5_month": "ALL",
        "tab5_exec": "ALL",
        ...
        # Which ONE extra filter to apply
        "tab5_filter_field": "ALL (no extra filter)",

        # Controls for the variable filter
        "tab5_addr_contains": "",
        "tab5_name_contains": "",
    ...
    if not supabase_connected:
        ...
        if df.empty:
            ...
            if missing:
                ...
                def make_label(row):
                    wf = str(row.get("flat_address", "") or "").strip()
                    nm = str(row.get("customer_name", "") or "").strip()
                    return f"{wf} - {nm}" if wf else nm

                # For customer dropdown inside the form,
                # build list based on current fixed filters
                ...
                label_map_preview = {
                    idx: make_label(r) for idx, r in base_preview.iterrows()
                }

                customer_options = ["ALL"] + [
                    label_map_preview[i] for i in base_preview.index
                ]
                ...
                with st.form("agreement_filter_form"):
                    ...
                    st.selectbox(
                        ...
                        options=var_fields,
                        index=(
                            var_fields.index(st.session_state.tab5_filter_field)
                    ...
                    with c1:
                        st.form_submit_button(
                            "Find",
                            on_click=_tab5_apply_filters,
                        )

                    with c2:
                        st.form_submit_button(
                            "Reset",
                            on_click=_tab5_reset_filters,
                        )

                # -------------------- RESULTS --------------------
                if not st.session_state.tab5_filters_active:
                    st.info(
                        "Set Month, Sales Executive & Wing (fixed), "
                        "choose one additional filter, then click **Find**."
                    )
                    ...
                    st.subheader(
                        f"Customers • {month_label} • {exec_label} • "
                        f"{wing_label} • {extra}"
                    )
                    ...
                    if post_df.empty:
                        ...
                        for idx, row in post_df.iterrows():
                            ...
                            (
                                ...
                                col10,
                                col11,
                            ) = st.columns([2, 3, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3])
                            ...
                            if is_referral_lead:
                                referral_checked = col7.checkbox(
                                    ...
                                    key=f"tab5_referral_{booking_id}",
                                )
                            else:
                            ...
                            insider_checked = col8.checkbox(
                                "Yes",
                                value=is_insider,
                                key=f"tab5_insider_{booking_id}",
                            )
                            ...
                            if has_offer1:
                                o1_checked = col10.checkbox(
                                    ...
                                    key=(
                                        f"tab5_offer1_{booking_id}_"
                                        f"{1 if is_o1_rewarded else 0}"
                                    ),
                                )
                            else:
                                col10.write("—")
                            ...
                            if has_offer2:
                                o2_checked = col11.checkbox(
                                    ...
                                    key=(
                                        f"tab5_offer2_{booking_id}_"
                                        f"{1 if is_o2_rewarded else 0}"
                                    ),
                                )
                            else:
                                col11.write("—")
                ...
                st.caption(
                    ...
                    "(always applied). Exactly one additional filter from the form "
                    "is applied at a time. Checkboxes update the Supabase table."
                )
with tab1:
    ...
    # ==========================================================
    if not supabase_connected:
        st.warning("🗄️ Please connect to Supabase to view dashboard data.")
        st.stop()
    ...
    # ==========================================================
    # SUPABASE → OLD GOOGLE SHEET COLUMN COMPATIBILITY MAPPING
    # This lets the existing Tab 1 logic keep using old names.
    # ==========================================================
    SUPABASE_TO_SHEET_COLS = {
        "id": "id",
    ...
    def prepare_booking_dashboard_df(source_df: pd.DataFrame) -> pd.DataFrame:
        """
        Converts Supabase bookings dataframe into the old Google Sheet-style
        dataframe expected by the existing Tab 1 dashboard code.
        """
        df = source_df.copy()

        # Rename Supabase snake_case columns to old dashboard column names
        df = df.rename(columns=SUPABASE_TO_SHEET_COLS)

        # Ensure required old-style columns exist
        required_defaults = {
            "Date": pd.NaT,
            "Customer Name": "",
            "Wing": "",
    ...
    # ==========================================================
    # DATA PREP FROM SUPABASE
    # ==========================================================
    df = prepare_booking_dashboard_df(booking_df)
    ...
    df["Quarter"] = df["Date"].apply(
        lambda x: get_custom_quarter_label(x) if pd.notna(x) else ""
    )
    ...
    # Clean Agreement Cost for PSF calculation
    df["Agreement Cost"] = (
        df["Agreement Cost"]
        .astype(str)
    ...
    df["Agreement Cost"] = pd.to_numeric(
        df["Agreement Cost"],
        errors="coerce"
    ).fillna(0.0)
    ...
    df["_FinalPrice_L"] = pd.to_numeric(fp_clean_all, errors="coerce")

    # Supabase numeric final_price may already be full amount.
    # Your old Google Sheet logic assumed lakhs.
    # This keeps old behavior but protects against already-full values.
    df["_FinalPrice_Full"] = df["_FinalPrice_L"].apply(
        lambda x: x * 100000 if pd.notna(x) and x < 10000 else x
    ).astype("float")
    ...
    if "Conversion Period (days)" in df.columns:
        conv_series = pd.to_numeric(
            df["Conversion Period (days)"],
            errors="coerce"
        )
    ...
    month_order = sorted([
        m for m in df["Month_dt"].unique()
        if pd.notna(m)
    ])

    ordered_months = [
        pd.to_datetime(str(m)).strftime("%B %y")
    ...
    # ==========================================================
    CARPET_SIZES = [
        480.94, 482.12, 545.00, 655.10, 665.65,
        666.29, 678.00, 689.00, 756.00, 790.00
    ]
    ...
    size_booked = (
        df.assign(
            _size=pd.to_numeric(
                df.get("Carpet Area", pd.Series(dtype="float")),
                errors="coerce"
            ).apply(nearest_size)
        )
        .dropna(subset=["_size"])
    ...
    WINGS_ORDER = ["E", "F", "B", "C"]
    WINGS = [
        w for w in WINGS_ORDER
        if any(k[0] == w for k in WING_TYPE_TOTALS.keys())
    ]
    ...
    # Pre-derive clean Type for floor-wise splits
    df["_NearestSize"] = pd.to_numeric(
        df.get("Carpet Area", 0),
        errors="coerce"
    ).apply(nearest_size)

    df["_DerivedType"] = df["_NearestSize"].map(SIZE_TYPE_MAP).fillna(
        df.get("Type", "")
    )
    ...
    def wing_type_chart(wing_df: pd.DataFrame, wing: str):
        ...
        bars = base.mark_bar(size=BAR_SIZE).encode(
            ...
            color=alt.Color(
                "Metric:N",
                scale=alt.Scale(domain=METRICS, range=METRIC_COLORS),
    ...
    # ==========================================================
    # PROJECTED SALES DASHBOARD CONFIG
    # ==========================================================
    ...
    class psd_FestDay:
        name: str
        d: date
        kind: str  # GOOD | BAD | HOLIDAY | INFO
    ...
    def psd_fixed_public_holidays(start: date, end: date) -> List[psd_FestDay]:
        ...
        for y in range(start.year, end.year + 1):
            fixed = [
                ("New Year", date(y, 1, 1)),
                ("Republic Day", date(y, 1, 26)),
                ("Shivaji Jayanti (MH)", date(y, 2, 19)),
                ("Ambedkar Jayanti", date(y, 4, 14)),
                ("Maharashtra Day", date(y, 5, 1)),
                ("Independence Day", date(y, 8, 15)),
                ("Gandhi Jayanti", date(y, 10, 2)),
    ...
    def psd_festival_days_between(start: date, end: date) -> List[psd_FestDay]:
        ...
        GOOD_DATES: Dict[int, List[Tuple[str, date]]] = {
            2026: [
                ("Nag Panchami", date(2026, 8, 17)),
                ("Onam", date(2026, 8, 26)),
                ("Ganesh Chaturthi / Ganesh Sthapana", date(2026, 9, 14)),
                ("Dhanteras", date(2026, 10, 29)),
                ("Diwali (Lakshmi Puja)", date(2026, 10, 31)),
            ],
            2027: [
                ("Makar Sankranti", date(2027, 1, 15)),
                ("Pongal", date(2027, 1, 15)),
                ("Vasant Panchami", date(2027, 2, 11)),
                ("Mahashivratri", date(2027, 3, 6)),
                ("Holika Dahan (before Holi)", date(2027, 3, 21)),
                ("Ugadi", date(2027, 4, 7)),
                ("Gudi Padwa", date(2027, 4, 7)),
                ("Chaitra Navratri Begins", date(2027, 4, 7)),
                ("Ram Navami", date(2027, 4, 15)),
                ("Akshaya Tritiya", date(2027, 5, 9)),
                ("Nag Panchami", date(2027, 8, 6)),
                ("Ganesh Chaturthi / Ganesh Sthapana", date(2027, 9, 4)),
                ("Onam", date(2027, 9, 12)),
                ("Dhanteras", date(2027, 10, 27)),
                ("Diwali (Lakshmi Puja)", date(2027, 10, 29)),
            ],
            2028: [
                ("Makar Sankranti", date(2028, 1, 15)),
                ("Pongal", date(2028, 1, 15)),
                ("Vasant Panchami", date(2028, 1, 31)),
                ("Mahashivratri", date(2028, 2, 23)),
                ("Holika Dahan (before Holi)", date(2028, 3, 10)),
                ("Ugadi", date(2028, 3, 27)),
                ("Gudi Padwa", date(2028, 3, 27)),
                ("Chaitra Navratri Begins", date(2028, 3, 27)),
                ("Ram Navami", date(2028, 4, 3)),
                ("Akshaya Tritiya", date(2028, 4, 27)),
                ("Nag Panchami", date(2028, 7, 26)),
                ("Ganesh Chaturthi / Ganesh Sthapana", date(2028, 8, 23)),
                ("Onam", date(2028, 9, 1)),
                ("Dhanteras", date(2028, 10, 15)),
                ("Diwali (Lakshmi Puja)", date(2028, 10, 17)),
            ],
            2029: [
                ("Makar Sankranti", date(2029, 1, 14)),
                ("Pongal", date(2029, 1, 14)),
                ("Vasant Panchami", date(2029, 1, 19)),
                ("Mahashivratri", date(2029, 2, 11)),
                ("Holika Dahan (before Holi)", date(2029, 2, 28)),
                ("Ugadi", date(2029, 4, 14)),
                ("Gudi Padwa", date(2029, 4, 14)),
                ("Chaitra Navratri Begins", date(2029, 4, 14)),
        ...
        # Chaitra Navratri windows
        for y in [2027, 2028, 2029]:
            begins = next(
                (
                    d for (n, d) in GOOD_DATES.get(y, [])
                    if "Chaitra Navratri Begins" in n
                ),
                None,
            )
            ...
            if begins and start <= begins <= end:
                psd_add_range(
                    ...
                    "Chaitra Navratri",
                    begins,
                    begins + timedelta(days=8),
                    "GOOD",
                )

        # Sharad Navratri + Dussehra
        sharad_starts = {
            2026: date(2026, 10, 11),
            2027: date(2027, 9, 30),
            2028: date(2028, 9, 19),
        }
        ...
        # Adhik Maas
        for s, e, nm in [
            (date(2026, 5, 17), date(2026, 6, 15), "Adhik Maas (Mal Maas)"),
        ...
        eclipse_bad = [
            ...
            ("Solar Eclipse", date(2027, 2, 6)),
            ("Lunar Eclipse", date(2027, 2, 21)),
            ("Solar Eclipse", date(2027, 8, 2)),
            ("Lunar Eclipse", date(2027, 8, 17)),
            ("Lunar Eclipse", date(2028, 1, 12)),
            ("Solar Eclipse", date(2028, 1, 26)),
    ...
    def psd_selling_days_count(s: date, e: date, by_date: Dict[date, List[psd_FestDay]]) -> int:
        return sum(
            1 for d in psd_iter_days(s, e)
            if psd_day_weight(d, by_date) > 0
        )

    # ==========================================================
    # SUPABASE BOOKING READER FOR PROJECTED SALES DASHBOARD
    # Replaces old Google Sheet worksheet reader.
    # ==========================================================
    @st.cache_data(ttl=120)
    def psd_read_booking_data_cached(input_df: pd.DataFrame) -> Tuple[int, Dict[Tuple[int, int], int]]:
        """
        Returns:
            total_bookings,
            achieved_by_month {(year, month): count}
        Uses the prepared dataframe from Supabase.
        """
        ...
        for _, r in temp.iterrows():
            ...
            ym = (int(d.year), int(d.month))
            achieved_by_ym[ym] = achieved_by_ym.get(ym, 0) + 1

        return len(temp), achieved_by_ym
    ...
    with TAB_BOOKING:
        ...
        if {"Date", "Quarter"}.issubset(df.columns):
            qorder_df = (
                df.dropna(subset=["Date", "Quarter"])
                  ...
                  .groupby("Quarter", as_index=False)[["_FY", "_QIDX"]]
                  .median()
            )
    
            ordered_quarters = (
                qorder_df.sort_values(["_FY", "_QIDX"])["Quarter"].tolist()
                if not qorder_df.empty
                else df["Quarter"].dropna().unique().tolist()
            )
        ...
        def _avg_psf_for(month_label: str, type_code=None):
        ...
        def _monthly_weighted_psf_stats(dataframe, type_code=None):
            ...
            ac = (
                pd.to_numeric(sub["Agreement Cost"], errors="coerce")
                if "Agreement Cost" in sub.columns
                else pd.Series(index=sub.index, dtype=float)
            )
    
            ca = (
                pd.to_numeric(sub["Carpet Area"], errors="coerce")
                if "Carpet Area" in sub.columns
                else pd.Series(index=sub.index, dtype=float)
            )
            ...
            month_tbl["WeightedAvgPSF"] = (
                month_tbl["TotalAgreementCost"] / month_tbl["TotalSaleableArea"]
            )
    
            if "ordered_months" in locals() and ordered_months:
                month_tbl["Month"] = pd.Categorical(
                    month_tbl["Month"],
                    categories=ordered_months,
                    ordered=True
                )
            ...
            appr_pct = (
                ((high_psf - low_psf) / low_psf * 100.0)
                if pd.notna(low_psf) and pd.notna(high_psf) and low_psf > 0
                else float("nan")
            )
        ...
        with r10c1:
            st.markdown(
                f"<div class='metric-card'><h3>Booking % — E & F</h3><p>{ef_pct:.1f}%</p></div>",
                unsafe_allow_html=True
            )
    
        with r10c2:
            st.markdown(
                f"<div class='metric-card'><h3>Booking % — B & C</h3><p>{bc_pct:.1f}%</p></div>",
                unsafe_allow_html=True
            )
    
        with r10c3:
            st.markdown(
                f"<div class='metric-card'><h3>Booking % — Phase 1</h3><p>{phase_pct:.1f}%</p></div>",
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            "<div class='section-subtitle'>🗓️ Quarter-wise Booking Count</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>📈 Quarter-wise Avg PSF (₹/sqft)</div>",
            unsafe_allow_html=True
        )
        ...
        if "Quarter" in df.columns:
            ...
            if not q_psf.empty:
                q_psf["Quarter"] = pd.Categorical(
                    q_psf["Quarter"],
                    categories=ordered_quarters,
                    ordered=True
                )
        ...
        st.markdown(
            "<div class='section-subtitle'>✅ Quarter-wise Agreement Done (Done only)</div>",
            unsafe_allow_html=True
        )
        ...
        if AGREE_COL not in df.columns:
            ...
            if "Quarter" not in df_ad.columns:
                if "Date" in df_ad.columns:
                    df_ad["Quarter"] = df_ad["Date"].apply(
                        lambda x: get_custom_quarter_label(x) if pd.notna(x) else ""
                    )
                else:
                    df_ad = pd.DataFrame()
                ...
                if df_done.empty:
                    ...
                    bar = alt.Chart(q_done).mark_bar().encode(
                        ...
                        tooltip=[
                            ...
                            alt.Tooltip("AgreementDone:Q", title="Done")
                        ]
                    )
                    ...
                    st.altair_chart(
                        (bar + labels).properties(
                            title=alt.TitleParams(
                                "Quarter-wise Agreement Done (Done)",
                                anchor="start",
                                fontSize=16,
                                fontWeight="bold",
                                dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>🏞️ Quarter-wise × Facing-wise Booking Count</div>",
            unsafe_allow_html=True
        )
        ...
        if {"Carpet Area", "Quarter"}.issubset(df.columns):
            ...
            if not df_face.empty:
                q_face_count = (
                    df_face.groupby(["Quarter", "Facing"])
                           .size()
                           .reset_index(name="Count")
                )
    
                q_face_count["Quarter"] = pd.Categorical(
                    q_face_count["Quarter"],
                    categories=ordered_quarters,
                    ordered=True
                )
                ...
                bar_qf = alt.Chart(q_face_count).mark_bar().encode(
                    ...
                    color=alt.Color(
                        "Facing:N",
                        scale=alt.Scale(domain=FACING_DOMAIN),
                        title="Facing"
                    ),
                ...
                st.altair_chart(
                    (bar_qf + lbl_qf).properties(
                        title=alt.TitleParams(
                            "Quarter-wise × Facing-wise Booking Count",
                            anchor="start",
                            fontSize=16,
                            fontWeight="bold",
                            dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>📅 Month-wise Bookings</div>",
            unsafe_allow_html=True
        )
    
        month_chart_data = pd.DataFrame({
            "Month": month_wise.index,
            "Count": month_wise.values
        })
        ...
        st.markdown(
            "<div class='section-subtitle'>📈 Month-wise Avg Rate (₹/sqft)</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>📅 Month-wise Carpet Area Sold (sq ft)</div>",
            unsafe_allow_html=True
        )
        ...
        if missing:
            ...
            if tmp.empty:
                ...
                if "ordered_months" in locals() and ordered_months:
                    ...
                    extras = [
                        m for m in plot_df[MONTH_COL].unique().tolist()
                        if m not in set(month_order)
                    ]
                    month_order = month_order + extras
                else:
                    month_order = (
                        plot_df.sort_values("CarpetAreaSold", ascending=False)[MONTH_COL]
                        .tolist()
                    )
    
                plot_df[MONTH_COL] = pd.Categorical(
                    plot_df[MONTH_COL],
        ...
        st.markdown(
            "<div class='section-subtitle'>📈 1 BHK & 2 BHK — Month-wise Avg PSF</div>",
            unsafe_allow_html=True
        )
    
        if {"Month", "Type", "Carpet Area", "Agreement Cost"}.issubset(df.columns):
            month_order_local = [
                m for m in ordered_months
                if m in df["Month"].unique().tolist()
            ]
            ...
            def _type_month_psf_chart(type_code: str, title: str):
                ...
                st.altair_chart(
                    (line + labels).configure_title(anchor="start"),
                    use_container_width=True
                )
        ...
        st.markdown(
            "<div class='section-subtitle'>🏢 Wing-wise Avg PSF by Quarter</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>🏢 Wing-wise Carpet Area Sold (sq ft)</div>",
            unsafe_allow_html=True
        )
        ...
        if missing:
            ...
            if tmp.empty:
                ...
                plot_df = (
                    tmp.groupby(WING_COL)[AREA_COL]
                       .sum()
                       .reset_index(name="CarpetAreaSold")
                )
    
                wing_order = (
                    plot_df.sort_values("CarpetAreaSold", ascending=False)[WING_COL]
                    .tolist()
                )
    
                plot_df[WING_COL] = pd.Categorical(
                    plot_df[WING_COL],
                    categories=wing_order,
                    ordered=True
                )
        ...
        st.markdown(
            "<div class='section-subtitle'>🏢 Wing-wise Bookings by Lead Type</div>",
            unsafe_allow_html=True
        )
        ...
        if missing:
            ...
            if tmp.empty:
                ...
                plot_df[LEAD_COL] = pd.Categorical(
                    plot_df[LEAD_COL],
                ...
                st.altair_chart(
                    (bars + labels).properties(
                        title=alt.TitleParams(
                            "Wing-wise Bookings by Lead Type (Grouped Bars)",
                            anchor="start",
                            fontSize=16,
                            fontWeight="bold",
                            dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>🏢 Sales Executive-wise Bookings by Wing (Stacked by Type)</div>",
            unsafe_allow_html=True
        )
            ...
            if tmp.empty:
                ...
                st.altair_chart(
                    (bars + total_labels).properties(
                        title=alt.TitleParams(
                            "Sales Executive-wise Bookings by Wing (Stacked by Type)",
                            anchor="start",
                            fontSize=16,
                            fontWeight="bold",
                            dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>👨‍💼 Sales Executive-wise Bookings</div>",
            unsafe_allow_html=True
        )
    
        exec_chart_data = pd.DataFrame({
            "Executive": sales_exec_wise.index,
            "Count": sales_exec_wise.values
        })
        ...
        st.markdown(
            "<div class='section-subtitle'>🧑‍💼 Sales Executive — Monthly Booking Count</div>",
            unsafe_allow_html=True
        )
        ...
        if "Sales Executive" in df.columns and not df.empty:
            ...
            if not se_month.empty:
                ...
                se_month["Month"] = pd.Categorical(
                    se_month["Month"],
                    categories=month_domain,
                    ordered=True
                )
    
                se_month["Sales Executive"] = pd.Categorical(
                    se_month["Sales Executive"],
                    categories=exec_order,
                    ordered=True
                )
                ...
                st.altair_chart(
                    (bars + labels).properties(
                        title=alt.TitleParams(
                            "Sales Executive — Monthly Bookings (Grouped Bars)",
                            anchor="start",
                            fontSize=16,
                            fontWeight="bold",
                            dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>👤 Avg Bookings per Month — Sales Executive</div>",
            unsafe_allow_html=True
        )
        ...
        if missing_cols_se:
            ...
            st.caption(
                f"SE avg/month diagnostics → unique SE: {dfx['Sales Executive'].nunique()}, "
                f"unique months: {dfx['MonthYear'].nunique()}, rows: {len(dfx)}"
            )
            ...
            if dfx.empty:
                ...
                if se_month.empty:
                    ...
                    bars = base.mark_bar(color="#10b981").encode(
                        y=alt.Y(
                            "Sales Executive:N",
                            sort=se_avg["Sales Executive"].tolist(),
                            title="Sales Executive"
                        ),
                        x=alt.X(
                            "AvgBookingsPerMonth:Q",
                            title="Avg Bookings / Month",
                    ...
                    st.altair_chart(
                        (bars + labels).properties(
                            title=alt.TitleParams(
                                "Avg Bookings per Month — Sales Executive",
                                anchor="start",
                                fontSize=16,
                                fontWeight="bold",
                                dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>⏱️ Sales Executive-wise Overall Average Conversion Period</div>",
            unsafe_allow_html=True
        )
    
        if "Conversion Period (days)" in df.columns:
            conv_tmp = df.copy()
            conv_tmp["ConvDays"] = pd.to_numeric(
                conv_tmp["Conversion Period (days)"],
                errors="coerce"
            )
        ...
        st.markdown(
            "<div class='section-subtitle'>🧑‍💼 Sales Executive-wise Bookings by Lead Type</div>",
            unsafe_allow_html=True
        )
        ...
        if missing:
            ...
            if tmp.empty:
                ...
                st.altair_chart(
                    (bars + labels).properties(
                        title=alt.TitleParams(
                            "Sales Executive-wise Bookings by Lead Type (Grouped Bars)",
                            anchor="start",
                            fontSize=16,
                            fontWeight="bold",
                            dy=-5
        ...
        st.markdown(
            "<div class='section-subtitle'>🧑‍💼 Sales Executive-wise Bookings by Visit Count</div>",
            unsafe_allow_html=True
        )
        ...
        if missing:
            ...
            if tmp.empty:
                ...
                plot_df["VisitBucket"] = pd.Categorical(
                    plot_df["VisitBucket"],
        ...
        st.markdown(
            "<div class='section-subtitle'>🏠 Type-wise Bookings</div>",
            unsafe_allow_html=True
        )
    
        type_chart_data = pd.DataFrame({
            "Type": type_wise.index,
            "Count": type_wise.values
        })
        ...
        type_bar = alt.Chart(type_chart_data).mark_bar(color="#f59e0b").encode(
            x=alt.X(
                "Type:N",
                title="Type",
                axis=alt.Axis(labelAngle=0, labelLimit=140, labelOverlap=True)
            ),
            y=alt.Y("Count:Q", title="Bookings"),
            tooltip=["Type", "Count"]
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>🧭 Bookings by Lead Type</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>⏱️ Bookings by Conversion Period (Spot & 2-month buckets)</div>",
            unsafe_allow_html=True
        )
        ...
        if CONV_COL not in df.columns:
            ...
            if conv_all.empty:
                ...
                sort_order = ["Spot Booking (0 days)"] + [
                    f"{2 * i}–{2 * (i + 1)} months"
                    for i in all_idx
                ]
                ...
                bars = base.mark_bar().encode(
                    ...
                    tooltip=[
                        alt.Tooltip("Bucket:N", title="Interval"),
                        alt.Tooltip("Bookings:Q", title="Bookings")
                    ]
                )
        ...
        st.markdown(
            "<div class='section-subtitle'>📈 Booking Count by Visit Count</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>📍 Location-wise Bookings</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='section-subtitle'>⏳ Time Taken per 50 Bookings</div>",
            unsafe_allow_html=True
        )
        ...
        df_bd["_NearestSizeBD"] = pd.to_numeric(
            df_bd.get("Carpet Area", 0),
            errors="coerce"
        ).apply(nearest_size)
        ...
        df_bd.loc[_bad, "_TypeForBD"] = (
            df_bd.loc[_bad, "_NearestSizeBD"]
            .map(SIZE_TYPE_MAP)
        )
        ...
        if "_FinalPrice_Full" not in df_bd.columns or "_FinalPrice_L" not in df_bd.columns:
            ...
            df_bd["_FinalPrice_Full"] = df_bd["_FinalPrice_L"].apply(
                lambda x: x * 100000 if pd.notna(x) and x < 10000 else x
            ).astype("float")
    
        st.markdown(
            "<div class='section-subtitle'>💰 Average Closing Price — 1BHK vs 2BHK (Final Price)</div>",
            unsafe_allow_html=True
        )
        ...
        avg_close_full = (
            ...
            .rename(columns={
                "_TypeForBD": "Type",
                "_FinalPrice_Full": "AvgCloseFull"
            })
        )
    
        if not avg_close_full.empty:
            avg_close_full["Label"] = avg_close_full["AvgCloseFull"].map(
                lambda v: f"₹{v:,.0f}"
            )
            ...
            st.altair_chart(
                (chart_avg_close + labels_avg_close).properties(
                    height=280
                ).configure_title(anchor="start"),
                use_container_width=True
            )
        ...
        ef_sub["_Negotiated"] = ef_sub.apply(
            lambda r:
                (COSTSHEET_1BHK_FULL - r["_FinalPrice_Full"])
                if r["_TypeForBD"] == "1BHK"
                else (
                    COSTSHEET_2BHK_FULL - r["_FinalPrice_Full"]
                    if r["_TypeForBD"] == "2BHK"
                    else float("nan")
                ),
            axis=1
        )
        ...
        nego_avg = (
            ...
            .rename(columns={
                "_TypeForBD": "Type",
                "_Negotiated": "AvgNegotiated"
            })
        )
    
        st.markdown(
            "<div class='section-subtitle'>📉 Average Negotiated Amount — E & F (₹)</div>",
            unsafe_allow_html=True
        )
    
        if not nego_avg.empty:
            nego_avg["Label"] = nego_avg["AvgNegotiated"].map(
                lambda v: f"₹{v:,.0f}"
            )
            ...
            st.altair_chart(
                (chart_nego_avg + labels_nego_avg).properties(
                    height=280
                ).configure_title(anchor="start"),
                use_container_width=True
            )
        ...
        def _top5_by_price_brackets(df_in: pd.DataFrame, type_name: str):
            sub = df_in[
                (df_in["_TypeForBD"] == type_name)
                & df_in["_FinalPrice_L"].notna()
            ].copy()
            ...
            return (
                counts.groupby("_hi", as_index=False)
                      .first()
                      .sort_values("_hi", ascending=False)
                      .head(5)
            )
        ...
        st.markdown(
            "<div class='section-subtitle'>📦 Highest Final-Price Brackets — 1BHK (Dynamic, Lakhs)</div>",
            unsafe_allow_html=True
        )
        ...
        if not top1_price.empty:
            ...
            c1 = alt.Chart(top1_price).mark_bar().encode(
                x=alt.X(
                    "Bracket:N",
                    title="Final Price (Lakhs)",
                    sort=list(top1_price["Bracket"])
                ),
                y=alt.Y("Count:Q", title="Bookings"),
                tooltip=[
            ...
            st.altair_chart(
                (c1 + t1).properties(height=280).configure_title(anchor="start"),
                use_container_width=True
            )
        else:
        ...
        st.markdown(
            "<div class='section-subtitle'>📦 Highest Final-Price Brackets — 2BHK (Dynamic, Lakhs)</div>",
            unsafe_allow_html=True
        )
        ...
        if not top2_price.empty:
            ...
            st.altair_chart(
                (c2 + t2).properties(height=280).configure_title(anchor="start"),
                use_container_width=True
            )
        else:
        ...
        st.markdown(
            "<div class='section-subtitle'>📈 1BHK & 2BHK — Booking-wise Agreement Cost (First → Latest)</div>",
            unsafe_allow_html=True
        )
        ...
        df_line["Agreement Cost"] = pd.to_numeric(
            df_line["Agreement Cost"],
            errors="coerce"
        )
    
        df_line["_NearestSizeBD"] = pd.to_numeric(
            df_line.get("Carpet Area", 0),
            errors="coerce"
        ).apply(nearest_size)
        ...
        df_line.loc[_bad_t, "_TypeForBD"] = (
            df_line.loc[_bad_t, "_NearestSizeBD"]
            .map(SIZE_TYPE_MAP)
        )
        ...
        # ============================================================
        # FINAL TABLES AT END OF BOOKING DASHBOARD
        # ============================================================
        ...
        if missing_tbl:
            ...
            tbl_df = df[
                ["Quarter", "Wing", "Sales Executive", "Agreement Cost", "Carpet Area"]
            ].copy()
            ...
            if tbl_df.empty:
                ...
                final_tbl["Quarter"] = pd.Categorical(
                    final_tbl["Quarter"],
                    categories=q_domain,
                    ordered=True
                )
    
                final_tbl["Wing"] = pd.Categorical(
                    final_tbl["Wing"],
                    categories=wing_order,
                    ordered=True
                )
                ...
                for wing in wing_order:
                    ...
                    wing_tbl = (
                        wing_tbl
                        .sort_values(["Quarter", "Sales Executive"])
                        .reset_index(drop=True)
                    )
    
                    wing_tbl_display = wing_tbl.copy()
    
                    wing_tbl_display["Agreement Cost Sold (₹)"] = (
                        wing_tbl_display["Agreement Cost Sold (₹)"]
                        .map(lambda x: f"₹{x:,.0f}")
                    )
    
                    wing_tbl_display["Saleable Area Sold (sq ft)"] = (
                        wing_tbl_display["Saleable Area Sold (sq ft)"]
                        .map(lambda x: f"{x:,.0f}")
                    )
    ...
    # ------------------------------------------------------------
    # TAB_AGREEMENT: Agreement Done Dashboard - SUPABASE COMPATIBLE
    # ------------------------------------------------------------
    with TAB_AGREEMENT:
        ...
        # ============================================================
        # BASE DATAFRAME: ONLY AGREEMENT DONE ROWS
        # ============================================================
        AGREE_COL = "Agreement Done"
        ...
        if AGREE_COL not in df.columns:
            ...
            if df_ag.empty:
                ...
                def _fmt_psf(x):
                    return "—" if pd.isna(x) else f"₹{x:,.0f}/sqft"
                ...
                def _avg_bookings_per_month_ag(dataframe, type_code=None):
                    ...
                    month_key = (
                        "MonthYear"
                        if "MonthYear" in sub.columns
                        else ("Month" if "Month" in sub.columns else None)
                    )
                ...
                def _monthly_weighted_psf_stats_ag(dataframe, type_code=None):
                    ...
                    needed = {"Month", "Agreement Cost", "Carpet Area"}
                    ...
                    month_order_local = ordered_months if "ordered_months" in globals() or "ordered_months" in locals() else []
                ...
                # ============================================================
                # ENSURE DATE / QUARTER / MONTH FIELDS
                # ============================================================
                if "Date" in df_ag.columns:
                    df_ag["Date"] = pd.to_datetime(df_ag["Date"], errors="coerce", dayfirst=True)
    
                if "Quarter" not in df_ag.columns and "Date" in df_ag.columns:
                    df_ag["Quarter"] = df_ag["Date"].apply(
                        lambda x: get_custom_quarter_label(x) if pd.notna(x) else ""
                    )
    
                if "Month" not in df_ag.columns and "Date" in df_ag.columns:
                    df_ag["Month"] = df_ag["Date"].dt.strftime("%B %y")
    
                if "MonthYear" not in df_ag.columns and "Date" in df_ag.columns:
                    df_ag["MonthYear"] = df_ag["Date"].dt.strftime("%B %y")
                ...
                if {"Date", "Quarter"}.issubset(df_ag.columns):
                    ...
                    ordered_quarters_ag = (
                        ...
                        if not qorder_df_ag.empty
                        else df_ag["Quarter"].dropna().unique().tolist()
                    )
    
                # Ordered months
                if "ordered_months" not in locals():
                    ordered_months = []
    
                ordered_months_ag = (
                    [m for m in ordered_months if "Month" in df_ag.columns and m in df_ag["Month"].dropna().unique().tolist()]
                    if ordered_months
                    else sorted(df_ag["Month"].dropna().astype(str).unique().tolist())
                    if "Month" in df_ag.columns
                    else []
                )
                ...
                total_carpet_area_ag = pd.to_numeric(
                    df_ag.get("Carpet Area", 0),
                    errors="coerce"
                ).fillna(0).sum()
                ...
                month_wise_ag = (
                    df_ag["Month"].value_counts().reindex(ordered_months, fill_value=0)
                    if "Month" in df_ag.columns and ordered_months
                    else df_ag["Month"].value_counts()
                    if "Month" in df_ag.columns
                    else pd.Series(dtype=int)
                )
    
                type_wise_ag = (
                    df_ag["Type"].value_counts()
                    if "Type" in df_ag.columns
                    else pd.Series(dtype=int)
                )
    
                sales_exec_wise_ag = (
                    df_ag["Sales Executive"].value_counts()
                    if "Sales Executive" in df_ag.columns
                    else pd.Series(dtype=int)
                )
    
                wing_wise_ag = (
                    df_ag["Wing"].astype(str).str.strip().value_counts()
                    if "Wing" in df_ag.columns
                    else pd.Series(dtype=int)
                )
                ...
                with r1[0]:
                    st.markdown(
                        f"<div class='metric-card'><h3>Total Agreement Done Bookings</h3><p>{total_bookings_ag}</p></div>",
                        unsafe_allow_html=True
                    )
                ...
                with r2c1:
                    st.markdown(
                        f"<div class='metric-card'><h3>Total Carpet Area Sold</h3><p>{total_carpet_area_ag:,.0f} sq ft</p></div>",
                        unsafe_allow_html=True
                    )
                with r2c2:
                    st.markdown(
                        f"<div class='metric-card'><h3>Total Agreement Cost Sold (₹)</h3><p>{total_agreement_cost_ag:,.0f}</p></div>",
                        unsafe_allow_html=True
                    )
                ...
                with r6c1:
                    st.markdown(
                        f"<div class='metric-card'><h3>Booking % — E & F</h3><p>{ef_pct_ag:.1f}%</p></div>",
                        unsafe_allow_html=True
                    )
                with r6c2:
                    st.markdown(
                        f"<div class='metric-card'><h3>Booking % — B & C</h3><p>{bc_pct_ag:.1f}%</p></div>",
                        unsafe_allow_html=True
                    )
                with r6c3:
                    st.markdown(
                        f"<div class='metric-card'><h3>Booking % — Phase 1</h3><p>{phase_pct_ag:.1f}%</p></div>",
                        unsafe_allow_html=True
                    )
                ...
                with r9c1:
                    st.markdown(
                        ...
                        f"<div class='metric-sub'>Lowest Avg Month ({low_month_1_ag or '—'}): {_fmt_psf(low_1_ag)} &nbsp;|&nbsp; "
                        f"Highest Avg Month ({high_month_1_ag or '—'}): {_fmt_psf(high_1_ag)}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                with r9c2:
                    st.markdown(
                        ...
                        f"<div class='metric-sub'>Lowest Avg Month ({low_month_2_ag or '—'}): {_fmt_psf(low_2_ag)} &nbsp;|&nbsp; "
                        f"Highest Avg Month ({high_month_2_ag or '—'}): {_fmt_psf(high_2_ag)}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                with r9c3:
                    st.markdown(
                        ...
                        f"<div class='metric-sub'>Lowest Avg Month ({low_month_all_ag or '—'}): {_fmt_psf(low_all_ag)} &nbsp;|&nbsp; "
                        f"Highest Avg Month ({high_month_all_ag or '—'}): {_fmt_psf(high_all_ag)}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                ...
                if "Quarter" in df_ag.columns:
                    ...
                    if not q_psf.empty:
                        q_psf["Quarter"] = pd.Categorical(
                            q_psf["Quarter"],
                            categories=ordered_quarters_ag,
                            ordered=True
                        )
                ...
                if "Quarter" in df_ag.columns:
                    q_domain = (
                        ordered_quarters_ag
                        if ordered_quarters_ag
                        else sorted(df_ag["Quarter"].dropna().unique().tolist())
                    )
                ...
                if {"Carpet Area", "Quarter"}.issubset(df_ag.columns):
                    ...
                    df_face = df_ag.copy()
                    df_face["Facing"] = df_face["Carpet Area"].apply(classify_facing_ag)
                    df_face = df_face.dropna(subset=["Facing", "Quarter"])
                ...
                month_chart_data = pd.DataFrame({
                    "Month": month_wise_ag.index,
                    "Count": month_wise_ag.values
                })
                ...
                if not df_ag.empty and "Month" in df_ag.columns:
                    monthly_psf_ag = (
                ...
                if {"Month", "Type", "Carpet Area", "Agreement Cost"}.issubset(df_ag.columns):
                    month_order_local = [
                        m for m in ordered_months
                        if m in df_ag["Month"].unique().tolist()
                    ]
                    ...
                    def _type_month_psf_chart_ag(type_code: str, title: str):
                        sub = df_m[tnorm.eq(type_code)].copy()
                    ...
                    _type_month_psf_chart_ag("1BHK", "1 BHK — Monthwise Avg PSF")
                    _type_month_psf_chart_ag("2BHK", "2 BHK — Monthwise Avg PSF")
                ...
                exec_chart_data = pd.DataFrame({
                    "Executive": sales_exec_wise_ag.index,
                    "Count": sales_exec_wise_ag.values
                })
                ...
                if "Sales Executive" in df_ag.columns and not df_ag.empty and "Month" in df_ag.columns:
                    se_month = (
                ...
                type_chart_data = pd.DataFrame({
                    "Type": type_wise_ag.index,
                    "Count": type_wise_ag.values
                })
                ...
                def _booking_line_chart_ag(sub_df: pd.DataFrame, title: str):
                    if sub_df.empty:
                        st.info(f"No data available for {title}.")
                        return
                ...
                df_1 = df_line[df_line["_TypeForBD"] == "1BHK"].copy()
                _booking_line_chart_ag(df_1, "1BHK — Booking-wise Agreement Cost")
    
                df_2 = df_line[df_line["_TypeForBD"] == "2BHK"].copy()
                _booking_line_chart_ag(df_2, "2BHK — Booking-wise Agreement Cost")
                ...
                if missing_tbl:
                    ...
                    if tbl_df.empty:
                        ...
                        q_domain = (
                            ordered_quarters_ag
                            if ordered_quarters_ag
                            else sorted(final_tbl["Quarter"].unique().tolist())
                        )
    ...
    # ------------------------------------------------------------
    # TAB_INVENTORY: Inventory Dashboard - SUPABASE COMPATIBLE
    # ------------------------------------------------------------
    with TAB_INVENTORY:
        ...
        required_inventory_vars = [
            ...
            "CARPET_LABELS",
            "wing_type_chart",
            "grouped_bar_with_gaps",
        ]
    
        missing_runtime_vars = [
            v for v in required_inventory_vars
            if v not in globals() and v not in locals()
        ]
    
        if missing_runtime_vars:
            st.error(
                "Inventory Dashboard cannot load because these setup variables are missing: "
                + ", ".join(missing_runtime_vars)
            )
            st.stop()
    
        if df.empty:
            st.info("No booking data available for Inventory Dashboard.")
            st.stop()
    
        # Normalize important fields locally
        df_inv = df.copy()
    
        if "Wing" not in df_inv.columns:
            df_inv["Wing"] = ""
    
        if "Type" not in df_inv.columns:
            df_inv["Type"] = ""
    
        if "Carpet Area" not in df_inv.columns:
            df_inv["Carpet Area"] = 0
    
        if "Merged Units" not in df_inv.columns:
            df_inv["Merged Units"] = ""
    
        df_inv["Wing"] = df_inv["Wing"].fillna("").astype(str).str.strip()
        df_inv["Type"] = df_inv["Type"].fillna("").astype(str).str.strip().str.upper()
        df_inv["Carpet Area"] = pd.to_numeric(df_inv["Carpet Area"], errors="coerce")
    
        # ------------------------------------------------------------
        # Wing Inventory vs Bookings
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🏢 Wing Inventory vs Bookings</div>",
            unsafe_allow_html=True
        )
    
        booked_by_wing = df_inv["Wing"].value_counts().to_dict()
    
        rows = []
        all_wings = sorted(set(list(booked_by_wing.keys()) + list(WING_TOTALS.keys())))
    
        for wing in all_wings:
            if not str(wing).strip():
                continue
        ...
        if wing_inv_df.empty:
            st.info("No wing inventory data available.")
        else:
        ...
        # ------------------------------------------------------------
        # Inventory by Type — separate chart for each wing
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🏠 Inventory by Type - Wing-wise</div>",
            unsafe_allow_html=True
        )
    
        def build_wing_type_inventory_df(df_in: pd.DataFrame, wing_: str) -> pd.DataFrame:
            sub = df_in[df_in["Wing"].astype(str).str.strip() == wing_].copy()
    
            if "Type" not in sub.columns:
                sub["Type"] = ""
    
            sub["Type"] = sub["Type"].fillna("").astype(str).str.strip().str.upper()
    
            booked_counts = (
                sub.groupby("Type")
                .size()
                .reindex(TYPE_DOMAIN, fill_value=0)
                .to_dict()
            )
        ...
        for wing in WINGS:
            st.markdown(
                f"<div class='section-subtitle'>🏢 Wing {wing} — Inventory by Type</div>",
                unsafe_allow_html=True
            )
    
            wing_df = build_wing_type_inventory_df(df_inv, wing)
    
            if wing_df.empty:
                st.info(f"No inventory type data available for Wing {wing}.")
            else:
                st.altair_chart(
                    wing_type_chart(wing_df, wing),
                    use_container_width=True
                )
        ...
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🪟 Podium Inventory by Type (Wing-wise)</div>",
            unsafe_allow_html=True
        )
        ...
        def podium_inventory_by_type(wing: str) -> pd.DataFrame:
            ...
            for t in TYPE_DOMAIN:
                sizes_t = [
                    s for s in PODIUM_SIZES
                    if SIZE_TYPE_MAP.get(s) == t
                ]
    
                total = sum(
                    int(CARPET_TOTALS.get((wing, s), 0))
                    for s in sizes_t
                )
    
                booked = sum(
                    int(size_booked.get((wing, s), 0))
                    for s in sizes_t
                )
        ...
        for wing in WINGS:
            ...
            if dfp.empty:
                st.info(f"No podium inventory data available for Wing {wing}.")
            else:
                chart_podium = grouped_bar_with_gaps(
                    dfp,
                    x_field="Type",
                    x_domain=TYPE_DOMAIN,
                    title=f"Wing {wing} — Podium Flats by Type"
                )
        ...
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🏞️ Inventory by Facing (Wing-wise)</div>",
            unsafe_allow_html=True
        )
        ...
        for wing in WINGS:
            ...
            for face in FACING_DOMAIN:
                ...
                rows_face += [
                    {"Wing": wing, "Facing": face, "Metric": "Booked", "Count": booked},
                    {"Wing": wing, "Facing": face, "Metric": "Available", "Count": available},
                    {"Wing": wing, "Facing": face, "Metric": "Total", "Count": total},
                ]
    
            df_face = pd.DataFrame(rows_face)
    
            if df_face.empty:
                st.info(f"No facing inventory data available for Wing {wing}.")
            else:
                chart_face = grouped_bar_with_gaps(
                    df_face,
                    x_field="Facing",
                    x_domain=FACING_DOMAIN,
                    title=f"Wing {wing} — Facing (Garden vs Road)"
                )
        ...
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>📐 Inventory by Carpet Area (Wing-wise)</div>",
            unsafe_allow_html=True
        )
        ...
        for wing in WINGS:
            ...
            if df_size.empty:
                st.info(f"No carpet-size inventory data available for Wing {wing}.")
            else:
                chart_size = grouped_bar_with_gaps(
                    df_size,
                    x_field="Size",
                    x_domain=CARPET_LABELS,
                    title=f"Wing {wing} — Carpet Sizes"
                )
        ...
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🔗 Merged Units Sold</div>",
            unsafe_allow_html=True
        )
        ...
        if MERGED_COL in df_inv.columns:
            mu = (
                df_inv[MERGED_COL]
                .dropna()
                .astype(str)
            ...
            # Compatibility: some pandas versions return column name as MERGED_COL already
            if MERGED_COL not in merged_df.columns:
                first_col = merged_df.columns[0]
                merged_df = merged_df.rename(columns={first_col: MERGED_COL})
    
            if merged_df.empty:
                st.info("No merged units data available.")
            else:
                chart_merged = alt.Chart(merged_df).mark_bar(size=28).encode(
                    x=alt.X(
                        f"{MERGED_COL}:N",
                        sort=MERGED_DOMAIN,
                        title="Merged Combination",
                ...
                st.altair_chart(
                    (chart_merged + merged_labels).configure_title(anchor="start"),
                    use_container_width=True
                )
        else:
            st.info('Column "Merged Units" not found, so Merged Units Sold chart was skipped.')
    
        # ------------------------------------------------------------
        # Wing-wise Merged Units Sold
        # ------------------------------------------------------------
        st.markdown(
            "<div class='section-subtitle'>🔗 Wing-wise Merged Units Sold</div>",
            unsafe_allow_html=True
        )
    
        if ("Wing" in df_inv.columns) and (MERGED_COL in df_inv.columns):
            df_mu = df_inv[["Wing", MERGED_COL]].dropna().copy()
            ...
            df_mu = df_mu[
                (df_mu["Wing"] != "")
                & (df_mu[MERGED_COL].isin(MERGED_DOMAIN))
            ]
    
            wing_domain = WINGS if WINGS else sorted(df_mu["Wing"].unique().tolist())
            ...
            if merged_wing_df.empty:
                st.info("No wing-wise merged units data available.")
            else:
    ...
    # ------------------------------------------------------------
    # TAB_SE_PERFORMANCE: Sales Performance Command Center - SUPABASE COMPATIBLE
    # Uses:
    #   bookings table -> df / sheet_df
    #   daily_visits table -> daily_visits_df
    # ------------------------------------------------------------
    with TAB_SE_PERFORMANCE:
        ...
        def _fallback_fmt_psf(x):
            return f"₹{x:,.0f}/sqft" if pd.notna(x) and x >= 0 else "—"
    
        def _fallback_avg_psf(sub):
            if sub is None or sub.empty:
                return float("nan")
    
            if "Agreement Cost" not in sub.columns or "Carpet Area" not in sub.columns:
                return float("nan")
    
            agreement_cost = pd.to_numeric(sub["Agreement Cost"], errors="coerce").fillna(0)
            carpet_area = pd.to_numeric(sub["Carpet Area"], errors="coerce").fillna(0)
    
            saleable_area = carpet_area * 1.38
            total_saleable_area = saleable_area.sum()
    
            if total_saleable_area <= 0:
                return float("nan")
    
            return float(agreement_cost.sum() / total_saleable_area)
    
        avg_psf_fn = avg_psf if "avg_psf" in globals() else _fallback_avg_psf
        fmt_psf_fn = fmt_psf if "fmt_psf" in globals() else _fallback_fmt_psf
    
        # =========================================================
        # Booking dataframe prep
        # =========================================================
        if "df" in globals() and isinstance(df, pd.DataFrame):
            df_spcc = df.copy()
        elif "sheet_df" in globals() and isinstance(sheet_df, pd.DataFrame):
            df_spcc = sheet_df.copy()
        else:
            df_spcc = pd.DataFrame()
        ...
        for old_col, new_col in booking_rename_map.items():
            if old_col in df_spcc.columns and new_col not in df_spcc.columns:
                df_spcc[new_col] = df_spcc[old_col]
    
        if "Date" not in df_spcc.columns:
            df_spcc["Date"] = pd.NaT
    
        df_spcc["Date"] = pd.to_datetime(df_spcc["Date"], errors="coerce", dayfirst=True)
    
        if "Month" not in df_spcc.columns:
            df_spcc["Month"] = df_spcc["Date"].dt.strftime("%B %y")
    
        if "MonthYear" not in df_spcc.columns:
            df_spcc["MonthYear"] = df_spcc["Month"]
        ...
        for col in [
            ...
            if col not in df_spcc.columns:
                df_spcc[col] = ""
    
        df_spcc["Month"] = df_spcc["Month"].apply(_safe_text)
        df_spcc["MonthYear"] = df_spcc["MonthYear"].apply(_safe_text)
        df_spcc["Sales Executive"] = df_spcc["Sales Executive"].apply(_safe_text)
        df_spcc["Type"] = df_spcc["Type"].apply(_safe_text)
        df_spcc["Lead Type"] = df_spcc["Lead Type"].apply(_safe_text)
    
        df_spcc["Agreement Cost"] = pd.to_numeric(df_spcc["Agreement Cost"], errors="coerce").fillna(0)
        df_spcc["Carpet Area"] = pd.to_numeric(df_spcc["Carpet Area"], errors="coerce").fillna(0)
    
        # =========================================================
        # Ordered months
        # =========================================================
        ordered_months_spcc = globals().get("ordered_months", [])
    
        if not ordered_months_spcc:
            month_values = [
                m for m in df_spcc["Month"].dropna().unique().tolist()
                if _safe_text(m) and _safe_text(m).upper() not in ("NAN", "NONE", "NULL")
            ]
            ordered_months_spcc = sorted(month_values, key=month_sort_key)
        ...
        start_month = fcol1.selectbox(
            "From Month",
            ordered_months_spcc,
            index=0,
            key="spcc_start_month"
        )
    
        end_month = fcol2.selectbox(
            "To Month",
            ordered_months_spcc,
            index=len(ordered_months_spcc) - 1,
            key="spcc_end_month"
        )
    
        try:
            si = ordered_months_spcc.index(start_month)
            ei = ordered_months_spcc.index(end_month)
        except ValueError:
        ...
        if si > ei:
            si, ei = ei, si
            start_month, end_month = ordered_months_spcc[si], ordered_months_spcc[ei]
    
        selected_months = ordered_months_spcc[si:ei + 1]
    
        selected_month_norms = {
            str(_strip_apostrophe(m)).strip().upper()
            for m in selected_months
        }
    
        selected_month_keys = {
            month_label_to_key(m)
            for m in selected_months
            if month_label_to_key(m)
        }
    
        # =========================================================
        # Booking dataframe period filter
        # =========================================================
        df_all = df_spcc.copy()
    
        df_all["_MonthNorm"] = df_all["Month"].apply(
            lambda x: str(_strip_apostrophe(x)).strip().upper()
        )
        ...
        # =========================================================
        # Daily visits dataframe from Supabase
        # =========================================================
        dv_period = pd.DataFrame()
    
        if "daily_visits_df" in globals() and isinstance(daily_visits_df, pd.DataFrame):
            dv = daily_visits_df.copy()
        else:
            dv = pd.DataFrame()
    
        if not dv.empty:
            for old_col, new_col in DAILY_VISITS_RENAME_MAP.items():
                if old_col in dv.columns and new_col not in dv.columns:
                    dv[new_col] = dv[old_col]
            ...
            # If Date column was absent but visit_date exists, use it
            if dv["Date_obj"].isna().all() and "visit_date" in dv.columns:
                dv["Date_obj"] = dv["visit_date"].apply(parse_date_text)
            ...
            if not dv.empty:
                ...
                zero_mask = dv["Total Revisits"].fillna(0).astype(int) == 0
                dv.loc[zero_mask, "Total Revisits"] = dv.loc[zero_mask, exec_revisit_cols].sum(axis=1)
                ...
                dv.loc[zero_mask_vis, "Total Visits"] = (
                    dv.loc[zero_mask_vis, source_cols].sum(axis=1)
                    + dv.loc[zero_mask_vis, "Total Revisits"]
                )
                dv["Total Visits"] = dv["Total Visits"].astype(int)
    
                dv["Month"] = dv["Month"].apply(_safe_text)
    
                dv["_MonthNorm"] = dv["Month"].apply(
                    lambda x: str(_strip_apostrophe(x)).strip().upper()
                )
    
                dv["_MonthKey"] = dv["Month"].apply(month_label_to_key)
    
                missing_month_key_mask = dv["_MonthKey"].eq("") | dv["_MonthKey"].isna()
                dv.loc[missing_month_key_mask, "_MonthKey"] = dv.loc[missing_month_key_mask, "Date_obj"].apply(
                    lambda d: d.strftime("%Y-%m")
                )
    
                missing_month_norm_mask = dv["_MonthNorm"].eq("") | dv["_MonthNorm"].isna()
                dv.loc[missing_month_norm_mask, "_MonthNorm"] = dv.loc[missing_month_norm_mask, "Date_obj"].apply(
        ...
        st.markdown(
            f"""
            <div class='chips'>
                <span class='chip'><span class='dot'></span> From: {start_month}</span>
                <span class='chip'><span class='dot'></span> To: {end_month}</span>
                <span class='chip ok'><span class='dot'></span> Booking Records: {len(df_period)}</span>
                <span class='chip ok'><span class='dot'></span> Daily Visit Rows: {len(dv_period) if not dv_period.empty else 0}</span>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        if not df_period.empty:
            if "_DerivedType" in df_period.columns:
                ...
                type_col = df_period["Type"].astype(str).str.upper().str.strip()
            ...
            df_period["_ConvDays"] = pd.to_numeric(
                df_period["Conversion Period (days)"],
                errors="coerce"
            )
    
            df_period["_MonthDisplay"] = df_period["Month"].astype(str).replace("", "Unknown")
    
            booking_execs = sorted([
                x for x in df_period["Sales Executive"].dropna().astype(str).unique().tolist()
                if _safe_text(x)
            ])
        else:
            booking_execs = []
        ...
        # =========================================================
        avg_conv_overall = (
            df_period["_ConvDays"].mean()
            if (not df_period.empty and df_period["_ConvDays"].notna().any())
            else float("nan")
        )
        ...
        ov1.markdown(
            f"<div class='metric-card'><h3>Total Visits</h3><p>{total_visits:,}</p><span>Daily visits table</span></div>",
            unsafe_allow_html=True
        )
        ...
        if dv_period.empty:
            ...
            calls_long = pd.concat(
                [
                    calls_exec_df[["Sales Executive", "Calls Answered"]]
                    .rename(columns={"Calls Answered": "Count"})
                    .assign(Type="Calls Answered"),
    
                    calls_exec_df[["Sales Executive", "Calls Unanswered"]]
                    .rename(columns={"Calls Unanswered": "Count"})
                    .assign(Type="Calls Unanswered"),
                ],
                ignore_index=True
            )
            ...
            calls_conv_long = pd.concat(
                [
                    calls_exec_df[["Sales Executive", "Total Calls"]]
                    .rename(columns={"Total Calls": "Count"})
                    .assign(Type="Total Calls"),
    
                    calls_exec_df[["Sales Executive", "Attended Visits"]]
                    .rename(columns={"Attended Visits": "Count"})
                    .assign(Type="Attended Visits"),
    
                    calls_exec_df[["Sales Executive", "Revisits"]]
                    .rename(columns={"Revisits": "Count"})
                    .assign(Type="Revisits"),
                ],
                ignore_index=True
            )
        ...
        if dv_period.empty:
            ...
            weekday_summary = (
                dv_period.groupby("DayName", as_index=False, observed=True)
                .agg({
                    "Total Visits": "sum",
                    BOOK_COL: "sum",
                    CANC_COL: "sum",
            ...
            exec_visits_long = pd.concat(
                [
                    exec_visits_df[["Sales Executive", "Attended Visits"]]
                    .rename(columns={"Attended Visits": "Count"})
                    .assign(Type="Attended Visits"),
    
                    exec_visits_df[["Sales Executive", "Revisits"]]
                    .rename(columns={"Revisits": "Count"})
                    .assign(Type="Revisits"),
                ],
                ignore_index=True
        ...
        if df_period.empty:
            ...
            for se in executives:
                ...
                psf_val = avg_psf_fn(sub) if bookings > 0 else float("nan")
                ...
                lead_counts = sub["_LeadType2"].value_counts().reset_index()
            ...
            se_display["Overall PSF"] = se_display["Overall PSF"].apply(fmt_psf_fn)
            ...
            month_booking_summary["_Sort"] = month_booking_summary["_MonthDisplay"].apply(
                lambda x: ordered_months_spcc.index(x) if x in ordered_months_spcc else 9999
            )
            ...
            bhk_long = pd.concat(
                [
                    se_summary_df[["Sales Executive", "1 BHK Sold"]]
                    .rename(columns={"1 BHK Sold": "Count"})
                    .assign(Type="1 BHK"),
    
                    se_summary_df[["Sales Executive", "2 BHK Sold"]]
                    .rename(columns={"2 BHK Sold": "Count"})
                    .assign(Type="2 BHK"),
                ],
                ignore_index=True
            )
            ...
            with st.expander("Open detailed executive cards"):
                for _, row in se_summary_df.iterrows():
                    ...
                    st.caption(
                        f"Bookings: {int(row['Bookings']):,} | Overall PSF: {fmt_psf_fn(row['Overall PSF'])}"
                    )
...
with tab16:
    ...
    def _is_done(x):
        return _safe_str(x).lower() == "done"

    def _is_received(x):
        return _safe_str(x).lower() == "received"
    ...
    with cf_subtab1:
        ...
        with t1:
            _cf_card(
                ...
                _fmt_money(total_collection_all),
                "cf-blue",
        ...
        with t3:
            _cf_card(
                "Total Due — All Wings",
                _fmt_money(total_due_all),
                "cf-amber",
                f"Due {total_due_pct_all:.1f}%"
            )
                ...
                if submitted:
                    if not form_customer:
                        ...
                        try:
                            ...
                            st.error(f"Could not update Booking sheet: {e}")
